...
class AgentsController < ApplicationController
  before_action :set_agent, only: [:show, :chat, :status]
  before_action :authenticate_user!, only: [:chat, :personal_stats]
  ...
  def feedback
    ...
    if params[:rating].present?
      update_agent_learning(@agent, interaction, params[:rating].to_i)
    end
  ...
  def get_user_preferences
    ...
    memories.each do |memory_content|
      if memory_content.is_a?(Hash)
        preferences.merge!(memory_content)
      end
    end
  ...
  def calculate_current_load(agent)
    ...
    
    {
      ...
      status: load_percentage > 80 ? 'high' : load_percentage > 50 ? 'medium' : 'low',
  ...
  def calculate_personal_stats(user)
    interactions = AgentInteraction.where(user: user)
  ...
  def get_agent_recommendations(user)
    ...
    user_interactions = AgentInteraction.where(user: user)
  ...
  def get_emotional_insights(user)
    emotion_distribution = AgentInteraction.where(user: user)
  ...
  def get_user_achievements(user)
    achievements = []
    interaction_count = AgentInteraction.where(user: user).count
    
    if interaction_count >= 10
      achievements << { name: "Conversationalist", description: "Had 10+ AI conversations" }
    end
    
    if interaction_count >= 50
      achievements << { name: "AI Enthusiast", description: "Had 50+ AI conversations" }
    end
    
    # Check for high ratings
    high_rated_count = AgentInteraction.where(user: user, rating: 4..5).count
    if high_rated_count >= 20
      achievements << { name: "AI Whisperer", description: "20+ highly rated conversations" }
    end
  ...
  def calculate_growth_metrics(user)
    interactions = AgentInteraction.where(user: user).order(:created_at)
  ...
  def calculate_average_session_length(user)
    ...
    sessions = AgentInteraction.where(user: user)
  ...
  def calculate_engagement_consistency(user)
    ...
    interactions_by_week = AgentInteraction.where(user: user)
